...
export default  {
  ...
  'bound agent calling method throwing an error': function(test) {
    ...
    test.deepEqual(value.message, "Boom", 'should call immediately a bound Agent method.');
  ...
  'bound agent with function throwing an exception': function(test) {
    test.expect(1);

    var value = 0;

    const aFunction = (request, response) => { throw new EvalError("Boom") },
        aBoundAgent = boundAgent(coordinator(), "test", aFunction),
        aResponse = response(_ => null, v => value = v, _ => null);

    aBoundAgent.askNow(2, aResponse);

    test.deepEqual(value.message, "Boom", 'should notify the exception.');
